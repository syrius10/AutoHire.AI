...
const Integrations = () => {
  ...
  const [githubData, setGitHubData] = useState<Array<{ id: string, name: string }>>([]);
  ...
  const handleFetchLinkedIn = () =>
    fetchLinkedInProfile("some-profile-id").then(setLinkedInData);
  const handleFetchGitHub = () => {
    fetchGitHubContributions("some-github-username").then((data) => {
      setGitHubData(data)
    });
  };
  const handleScheduleEvent = () =>
    scheduleGoogleCalendarEvent(event).then(() => alert("Event Scheduled!"));
