...
export const analyzeInterviewQnA = async (transcript) => {
    try {
        ...
        return response.data;   // { insights: "Candidate provided strong leadership examples." }
    } catch (error) {
...
export const analyzeSentiment = async (audioFile) => {
    try {
      ...
      const response = await axios.post(`${AI_SENTIMENT_URL}/analyze_sentiment`, formData, {
...
export const analyzeVideo = async (videoFile) => {
    try {
      ...
      const response = await axios.post(`${AI_VIDEO_ANALYSIS_URL}/analyze-video`, formData, {
        headers: { "Content-Type": "multipart/form-data" },
      });
...
export const analyzeInterviewQnA = async (question) => {
...
export const analyzeSentiment = async (audioFile) => {
  ...
  const response = await axios.post(`${API_URL}/sentiment`, formData);
  return response.data;
};
