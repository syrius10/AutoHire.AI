...
export const analyzeResumeSkills = async (resume, jobDescription) => {
    try {
        const response = await axios.post(`${API_URL}/analyze`, { resume, job_description: jobDescription });
...
export const getTrainingRecommendations = async (missingSkills) => {
    try {
        const response = await axios.post(`${API_URL}/recommendations`, { missingSkills });
        return response.data;
    } catch (error) {
