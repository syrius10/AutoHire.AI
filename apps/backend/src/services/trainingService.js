...
export const getTrainingRecommendations = async (missingSkills) => {
    try {
        const courseraCourses = await axios.get(COURSERA_API, { params: { query: missingSkills.join(",") } });
        const udemyCourses = await axios.get(UDEMY_API, { params: { search: missingSkills.join(",") } });
