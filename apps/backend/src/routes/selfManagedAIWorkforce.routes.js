...
router.post("/decision", async (req, res) => {
    try {
        const { performance, resourceUtilization, efficiency, errorRate } = req.body;

        if (!performance || !resourceUtilization || !efficiency || errorRate === undefined) {
            return res.status(400).json({ error: "Missing required fields" });
        }

        const decision = await manageSelfAIWorkforce(performance, resourceUtilization, efficiency, errorRate);
        res.json({ decision });
    } catch (error) {
